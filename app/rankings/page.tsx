  ...
  return (
    <main className="ranking-page">
      ...
      <section className="ranking-lower-grid">
        <article className="ranking-panel"><h2>Validasi sinyal (neto)</h2>{summary?.sampleSize ? <><div className="backtest-metrics"><div><span>Sample</span><strong>{summary.sampleSize}</strong></div><div><span>Win rate 10D</span><strong>{percent(summary.winRate10d)}</strong></div><div><span>Expectancy 10D</span><strong>{summary.expectancy10d == null ? '—' : `${summary.expectancy10d.toFixed(2)}%`}</strong></div><div><span>Max drawdown</span><strong>{summary.maxDrawdown10d == null ? '—' : `${summary.maxDrawdown10d.toFixed(2)}%`}</strong></div></div><p className="ranking-note">Biaya: beli {summary.costAssumptions.buyFeePercent}% + jual {summary.costAssumptions.sellFeePercent}%, slippage {summary.costAssumptions.slippagePercentPerSide}% per sisi. Return bruto rata-rata: {summary.grossAverageReturn10d == null ? '—' : `${summary.grossAverageReturn10d.toFixed(2)}%`}.</p></> : <p>Outcome belum cukup. Evaluator akan mengisi hasil setelah sinyal berumur 20 sesi perdagangan.</p>}</article>
