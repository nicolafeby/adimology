...
export function roundIdxPrice(value: number | null, mode: 'down' | 'up' | 'nearest' = 'nearest'): number | null {
  ...
  let rounded = value;
  for (let i = 0; i < 2; i++) { const tick = getFraksi(rounded); rounded = mode === 'down' ? Math.floor(value / tick) * tick : mode === 'up' ? Math.ceil(value / tick) * tick : Math.round(value / tick) * tick; }
...
export function calculateTradingDecision(input: DecisionInput): TradingDecision {
  ...
  const atrValue = current !== null && effectiveVolatilityPct !== null ? current * effectiveVolatilityPct / 100 : null;
  ...
  const atrStop = reference !== null && atrValue !== null ? reference - atrValue * DECISION_THRESHOLDS.atrMultiplier : null;
  const stopPrice = roundIdxPrice(atrValue === null ? null : supportStop !== null && atrStop !== null ? Math.min(supportStop, atrStop) : supportStop ?? atrStop, 'down');
  ...
  const accountSize = finite(input.sizing?.accountSize), maxRiskPct = finite(input.sizing?.riskPercent);
  if (accountSize !== null && accountSize > 0 && maxRiskPct !== null && maxRiskPct > 0 && risk !== null && risk > 0 && reference !== null) { const riskBudget = accountSize * Math.min(100, maxRiskPct) / 100; const shares = Math.floor(Math.min(riskBudget / risk, accountSize / reference) / LOT_SIZE) * LOT_SIZE; positionSizing = { riskPerShare: risk, riskBudget, maximumShares: shares, maximumLots: shares / LOT_SIZE, positionValue: shares * reference, positionRisk: shares * risk }; }
