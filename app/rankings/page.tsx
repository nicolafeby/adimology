  ...
  return (
    <main className="ranking-page">
      ...
      <section className="ranking-lower-grid">
        <article className="ranking-panel"><h2>Validasi sinyal</h2>{summary?.sampleSize ? <div className="backtest-metrics"><div><span>Sample</span><strong>{summary.sampleSize}</strong></div><div><span>Win rate 10D</span><strong>{percent(summary.winRate10d)}</strong></div><div><span>Avg return 10D</span><strong>{summary.averageReturn10d == null ? '—' : `${summary.averageReturn10d.toFixed(2)}%`}</strong></div><div><span>Target hit</span><strong>{percent(summary.targetHitRate)}</strong></div></div> : <p>Outcome belum cukup. Evaluator akan mengisi hasil setelah sinyal berumur 20 sesi perdagangan.</p>}</article>
